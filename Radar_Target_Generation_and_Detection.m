...
Bsweep = c/(2*rangeResolution); %Bsweep
Tchirp = 5.5*2*range_max/c; %sec 5.5 is used (should be at least between 5 and 6
Slope = Bsweep/Tchirp; 
...
for i=1:length(t)         
    ...
    r_t(i) = range_i + vel_i*t(i); %range of the target
...
beat_f = reshape(Mix, [Nr, Nd]);

 % *%TODO* :
%run the FFT on the beat signal along the range bins dimension (Nr) and
%normalize.
beat_fft = fft(beat_f, Nr);
beat_fft = beat_fft./Nr;

 % *%TODO* :
% Take the absolute value of FFT output
beat_abs = abs(beat_fft);

 % *%TODO* :
% Output of FFT is double sided signal, but we are interested in only one side of the spectrum.
% Hence we throw out half of the samples.
beat_final = beat_abs(1:Nr/2);
...
N_Train = (2*Tr+2*Gr+1)*(2*Td+2*Gd+1) - (2*Gr+1)*(2*Gd+1);
...
figure,surf(doppler_axis,range_axis,RDM_filtered);
colorbar;
